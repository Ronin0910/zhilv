...
// 获取静态地图图片URL
const getStaticMapUrl = async (): Promise<string | null> => {
  ...
  tripPlan.value.days.forEach(day => {
    day.attractions.forEach(attraction => {
      if (attraction.location && attraction.location.longitude && attraction.location.latitude) {
        locations.push(`${attraction.location.longitude},${attraction.location.latitude}`)
  ...
  try {
    ...
    const response = await fetch(`/api/map/static-map?${params}`)
...
const replaceMapWithStatic = async (exportContainer: HTMLElement) => {
  ...
  const staticMapUrl = await getStaticMapUrl()

  if (staticMapUrl) {
    // 使用静态地图图片
    mapContainer.innerHTML = `
      <img src="${staticMapUrl}" style="width:100%;height:100%;object-fit:cover;border-radius:8px;" />
    `
  } else {
    // 如果获取静态地图失败，显示景点列表
    const attractionsHtml = tripPlan.value?.days.map((day, dayIndex) =>
      day.attractions.map((attr, attrIndex) => `
        <div style="display:flex;align-items:center;padding:8px;border-bottom:1px solid #eee;">
          <div style="width:24px;height:24px;background:#4CAF50;color:white;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:12px;font-weight:bold;margin-right:8px;">${attrIndex + 1}</div>
          <div>
    ...
    mapContainer.innerHTML = `
      <div style="padding:16px;background:#f5f7fa;border-radius:8px;">
        <h4 style="margin:0 0 12px 0;color:#333;">📍 景点列表</h4>
        ${attractionsHtml}
      </div>
    `
  }
